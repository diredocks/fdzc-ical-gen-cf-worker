    ...
    text({ text }) {
      const trimmed =
        text.trim()
          .replace('&nbsp;', '');
      if (trimmed)
        leftCurrentCourse.push(trimmed);
    },
  })
  // 右表格
...
// 从左表格提取信息，弄成一个 Map
leftTable = new Map(leftTable.map(v => {
  const name = v[0].replaceAll(' ', '');
  const weeks =
    v.filter(item => item.includes('～')).flatMap(str => {
...
rightTable = rightTable.flatMap(row => {
  ...
  const groupedInfo =
    splitBy(rest.map(parseInfoToken), token => leftTable.has(token.name))
